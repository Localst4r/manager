...
function formatHand(hand, hideSecond = false) {
  return hand.map((c, i) => (hideSecond && i === 1 ? 'ðŸ‚ ' : `${c.val}${c.suit}`)).join(' ');
}
...
function buildEmbed(game, result = null) {
  ...
  if (result === 'win') { color = 0x57f287; title = 'ðŸƒ Blackjack - You Won!'; }
  else if (result === 'lose') { color = 0xe74c3c; title = 'ðŸƒ Blackjack - You Lost'; }
  else if (result === 'push') { color = 0xf39c12; title = 'ðŸƒ Blackjack - Push (Tie)'; }
  else if (result === 'blackjack') { color = 0xf1c40f; title = 'ðŸƒ Blackjack - BLACKJACK!'; }
  else if (result === 'bust') { color = 0xe74c3c; title = 'ðŸƒ Blackjack - Bust!'; }
