...
function navigateTo(page) {
  ...
  if (page === 'permissions') populatePermissions();
}
...
async function loadHome() {
  ...
  list.innerHTML = lb.slice(0, 10).map((u, i) => `
    <div class="lb-item">
      ...
      <div class="lb-coins">ðŸª™ ${u.balance.toLocaleString()}</div>
    </div>
  `).join('');
}
...
async function populatePermissions() {
  if (!allCommands.length) {
    ...
    const select = document.getElementById('perm-command-select');
...
function loadCommandPerms() {
  const cmd = getSelectVal('perm-command-select');
  if (!cmd) {
    setVal('perm-roles', '');
    setVal('perm-users', '');
    return;
  }
  const perms = (config.commandPerms || {})[cmd] || {};
  setVal('perm-roles', (perms.roles || []).join(', '));
  setVal('perm-users', (perms.users || []).join(', '));
}

async function saveCommandPerms() {
  const cmd = getSelectVal('perm-command-select');
  if (!cmd) return alert('select a command first');

  const roles = getVal('perm-roles').split(',').map(s => s.trim()).filter(Boolean);
  const users = getVal('perm-users').split(',').map(s => s.trim()).filter(Boolean);

  if (!config.commandPerms) config.commandPerms = {};
  config.commandPerms[cmd] = { roles, users };

  if (!roles.length && !users.length) {
    delete config.commandPerms[cmd];
  }

  await api('/config', 'POST', { commandPerms: config.commandPerms });
